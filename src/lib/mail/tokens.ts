...
export async function getAccessToken(userId: string, provider: MailProvider): Promise<{ token: string; email: string | null }> {
  const conn = (await getUserConnections(userId)).find((c) => c.provider === provider);
  if (!conn) throw new MailError("not_connected", "No mailbox connected yet.");

  if (provider === "google" && conn.scopes.length > 0 && !conn.scopes.includes(GMAIL_SCOPE)) {
    throw new MailError(
      "missing_scope",
...
export async function connectionStatus(userId: string) {
  ...
  return conns.map((c) => {
    ...
    if (c.provider === "google" && c.scopes.length > 0 && !c.scopes.includes(GMAIL_SCOPE)) status = "missing_scope";
